...
2.  **Set up the target workbook**:
    ...
    - Define the `targetFileName` (e.g., "target_workbook.xlsm").
...
3.  **Open and process purchase files**:
    ...
    - Use the `Dir` function to get the name of the first file in the `folderPath` that matches a specific pattern (`"2024-*\ _purchases"`). The asterisk (`*`) acts as a wildcard. This pattern is designed to identify all the monthly purchases files (e.g., "2024-01_purchases", "2024-02_purchases", etc.).
